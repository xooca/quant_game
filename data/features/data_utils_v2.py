...
class initial_data_setup(DefineConfig):
    def __init__(self,master_config_path,db_conection):
    ...
    def unzip_folders(self):
        self.f_name_list = []
        if not ddu.check_if_table_exists(self.db_conection,table_name=self.available_zip_files_table):
            df_empty = pd.DataFrame(columns=['timestamp','file_name','status'])
            df_empty['timestamp']=pd.to_datetime(df_empty["timestamp"])
            ddu.create_table(self.db_conection,table_name=self.ohlc_raw_data_table,create_table_arg={'replace':True},df=df_empty)
            ...
            for filename in fnmatch.filter(files, self.zip_file_pattern):
                ...
                if zipfile.is_zipfile(f_name):
                    try:
                        ...
                        ct = datetime.now()
                        insert_arg = {'insert_values': f"('{ct}','{f_name}','unzipped'"}
                        ddu.insert_data(self.db_conection,table_name=self.loaded_source_files_table,insert_arg=insert_arg,df=None)
                    except Exception as e1:
                        print_log(f"File {f_name} is not unzipped",self.using_print)
                        print_log(f"Error encountered is {e1}",self.using_print)
                        ct = datetime.now()
                        insert_arg = {'insert_values': f"('{ct}','{f_name}','{e1}'"}
                        ddu.insert_data(self.db_conection,table_name=self.loaded_source_files_table,insert_arg=insert_arg,df=None)
                else:
                    print_log(f"File {f_name} is not unzipped",self.using_print)
    ...
    def create_dataset(self,reload_all = True):
        ...
        if not ddu.check_if_table_exists(self.db_conection,table_name=self.ohlc_raw_data_table):
            df_empty = pd.DataFrame(columns=['timestamp','ticker','open','high','low','close'])
            df_empty['timestamp']=pd.to_datetime(df_empty["timestamp"])
            ddu.create_table(self.db_conection,table_name=self.ohlc_raw_data_table,create_table_arg={'replace':True},df=df_empty)

        try:
            self.loaded_source_files_table_df = ddu.load_table_df(self.db_conection,table_name=self.loaded_source_files_table) 
        except Exception as e1:
            df_empty = pd.DataFrame(columns=['timestamp','file_name','status'])
            df_empty['timestamp']=pd.to_datetime(df_empty["timestamp"])
            ddu.create_table(self.db_conection,table_name=self.loaded_source_files_table,create_table_arg={'replace':True},df=df_empty)
            #create_table(con,table_name='tester.ohlc',create_table_arg={'replace':True})

        if self.loaded_source_files_df.shape > 0 and reload_all is False:
            already_loaded_files = set(self.loaded_source_files_table_df['file_name'].to_list())
        for root, dirs, files in os.walk(self.raw_data_input_path):
            for filename in fnmatch.filter(files, self.data_pattern):
                    try:
                        f_name = Path(os.path.join(root, filename))
                        if f_name not in already_loaded_files:
                            ...
                            print_log(f"Data loaded from file {f_name} and has shape {tmp_df.shape}",self.using_print)
                            ddu.insert_data(self.db_conection,table_name=self.ohlc_raw_data_table,insert_arg={},df=tmp_df)
                            ct = datetime.now()
                            insert_arg = {'insert_values': f"('{ct}','{f_name}','loaded'"}
                            ddu.insert_data(self.db_conection,table_name=self.loaded_source_files_table,insert_arg=insert_arg,df=None)
                    except Exception as e:
                            print_log(f"Error while writing file to {self.loaded_source_files_table} : {e}",self.using_print)
                            print_log(f"Unable to write {f_name} to the table",self.using_print)
                            ct = datetime.now()
                            insert_arg = {'insert_values': f"('{ct}','{f_name}','{e}'"}
                            ddu.insert_data(self.db_conection,table_name=self.loaded_source_files_table,insert_arg=insert_arg,df=None)
