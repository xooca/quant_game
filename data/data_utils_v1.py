...
import re
import gc
from config.common import Config,DefineConfig
...
class execute_data_pipeline(DefineConfig):
    def __init__(self,master_config_path):
        ...
        print_log(f"Feature spec file is {self.datapipeline_spec}",self.using_print)
        feature_spec = importlib.import_module(f"{self.datapipeline_spec}")
        self.feature_pipeline = feature_spec.pipelines(self.config) 
        self.base_df = pd.DataFrame()
    
    def run_individual_pipeline(self,pipe_name,pipe,pipe_location,datapipeline,pipe_data_location):
    ...
    def run_pipeline(self,datapipeline,subdatapipeline,all_func):
            ...
            if len(all_pipe)>0:
                ...
                if self.load_previous:
                    if os.path.exists(flag_file):
                        ...
                        return
                pipe_data_location = f"{self.base_data_loc}{datapipeline}.csv"
                for pipe_name, pipe in zip(name_pipe_list,all_pipe):
                    self.run_individual_pipeline(pipe_name,pipe,self.base_data_loc,datapipeline,pipe_data_location)
    ...
    def run_initial_pipeline(self,initial_pipeline_save_path,df=None):
        if df is not None:
            initial_df = pd.read_csv(self.input_path)
        else:
            initial_df = df
        ...
        check_and_create_dir(initial_pipeline_save_path)
    ...
    def run_main_pipeline(self,initial_pipeline_save_path):
        ...
        for datapipeline,subdatapipeline in pipelines_dict.items():
            self.run_pipeline(datapipeline,subdatapipeline,all_func)
    ...
    def merge_data(self):
        master_pipeline_path = f"{self.base_data_loc}{self.master_pipeline}.csv"
        ...
        master_df_col = [col for col in master_df.columns.tolist() if col not in self.pipeline1_exclude_cols]
        print_log(f"Columns excluded : {self.pipeline1_exclude_cols}",self.using_print)
        master_df = master_df[master_df_col]
        
        for pipeline in self.merge_pipeline_to_master. :
            tmppath = f"{self.base_data_loc}{pipeline}.csv"
